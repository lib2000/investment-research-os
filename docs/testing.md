...
Firecrawl Monitor 변화 감지는 `python tools\check_firecrawl_monitor_collector.py --input-json docs\examples\firecrawl_monitor_registry.sample.json`로 페이지/검색 target, schedule, goal, changeTracking payload를 먼저 확인합니다. 실제 `/v2/monitor` 생성 전에는 ignored secret env 파일을 만든 뒤 `--require-env-registry --require-webhook-secret --require-create-ready`를 통과시켜야 합니다. 이때 `monitor_webhook_configured`와 `monitor_webhook_count`는 Firecrawl monitor payload가 이벤트를 우리 webhook endpoint로 보낼 준비가 됐는지 확인하는 별도 신호입니다. 이벤트 수신 모델은 `CompanyIrSourcesWatchTests`의 Firecrawl Monitor 이벤트 정규화/저장/엔드포인트 테스트로 `monitor.page`, `monitor.check.completed`, 중복 upsert, 공개 IR/SEC 상태 연결을 확인합니다. Webhook 테스트는 secret 미설정, secret 불일치, `Authorization: Bearer` 정상 수신 세 가지를 고정합니다.
