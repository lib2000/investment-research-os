...
- 최신성 가드: 추천 저장 점검은 `Asia/Seoul` 날짜 기준으로 최신 추천일이 허용 범위 안에 있고, 해당 일자 추천이 정확히 3개인지 확인한다.
...
- 백엔드가 꺼진 상태에서는 `python tools\check_research_source_store.py --strict`로 KCIF, EMERiCs/CSF/KIEP, 네이버 리서치, 신한 리서치, 마감 시황 시장일지, 티커 레지스트리, 중복 Dossier 큐 캐시 상태를 먼저 확인한다. 이 점검은 마감 시황 자동 수집 시도 상태와 리서치 자동화 Dossier 갱신 상태도 함께 확인한다.
...
- 오프라인 포트폴리오 가드는 `updated_at`, 가격 확인 시각, 해외/수동 수량 `sync_checked_at`, 비중 합계, 저장 총액과 종목 평가금액 합계까지 함께 확인한다. 종목별로 평가금액, 투자금, 수익, 수익률 계산도 재검산하며 해외 종목은 평가/투자금에 적용된 환율이 서로 크게 어긋나지 않는지 확인한다.
