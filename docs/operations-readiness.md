...
- 백엔드가 꺼진 상태에서는 `python tools\check_portfolio_store.py --portfolio 이형주 --min-holdings 17 --forbid-zero`와 핵심 해외/수동 보유 종목 기대값으로 저장 원본 수량을 먼저 확인한다. 국내 동기화 보유 종목 수는 실제 매수·매도에 따라 변하므로 과거의 정확 개수로 고정하지 않는다.
...
- `현재 작업 디렉토리가 없습니다` 또는 OneDrive 경로가 보이면 PowerShell에서 `. D:\workspace\InvestmentJournalApp\scripts\enter-investment-research-os.ps1`를 실행해 현재 창의 작업 루트를 바로잡는다.
- 콘솔 주소는 `http://127.0.0.1:8001/console/index.html`이고, 백엔드는 `D:\workspace\InvestmentJournalApp`에서 `.\scripts\start-research-backend.ps1 -Port 8001`로 실행한다.
