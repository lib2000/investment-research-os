...
- Market Signal Graph pipeline contract: `market_signal_graph_pipeline_contract_v1`은 Firecrawl IR payload, Firecrawl earnings payload, earnings transcript payload, DeepSeek IR analysis payload, IR/Earnings/SEC/DART 통합 점수, portfolio health 변화 감지, Telegram brief dry-run을 하나의 offline contract로 묶는다. 외부 RPC, Firecrawl, DeepSeek, Telegram 전송은 호출하지 않고 shape/count/status drift만 검증한다.
...
전체 클릭 스모크는 실제 메뉴/버튼/포트폴리오/LLM/RAG/추천 추적까지 확인하므로 수 분이 걸릴 수 있다. 자동화나 터미널 래퍼에서 실행할 때는 외부 명령 제한 시간을 최소 600초 이상으로 두고, `--progress`로 주요 진행 구간을 출력한다.
