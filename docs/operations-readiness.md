...
- Firecrawl Monitor 운영 preflight: 실전 전환 전에는 `python tools\check_firecrawl_monitor_operational_preflight.py --env-file path\to\firecrawl-monitor.env --env-override --require-env-registry --require-webhook-secret`를 먼저 실행한다. 이 검사는 secret과 registry를 확인하고, 임시 vault에서 webhook 불일치 401 및 정상 수신 200/저장 1건 흐름을 검증한다. 실제 monitor 생성 직전에는 추가로 `--require-create-ready`를 붙인다.
- Firecrawl Monitor 생성 readiness 리포트: `--readiness-report output\firecrawl_monitor_create_readiness_report.json`을 붙이면 실제 `/v2/monitor` 생성 없이 조건, 차단 사유, monitor별 target 수/type/schedule/webhook 여부/payload hash prefix만 저장한다. placeholder API key나 placeholder webhook secret은 configured로 보지 않는다.
