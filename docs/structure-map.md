...
| `backend\research_os\dart_watch_universe.py` | DART 감시 universe, 일일 점검 상태, 활성 실패 필터 helper | `DartFilingWatchTests` |
...
| `backend\research_os\daily_recommendations.py` | 매일 한국/미국 추천 1~3위 저장, 스케줄 상태, 추천 후보 저장 품질 점수 보정 | `check_daily_recommendations_store.py`, `check_daily_recommendation_render_layout.py` |
...
| `backend\research_os\alpha_vantage_data_provider.py` | Alpha Vantage 회사 개요 보강 provider | `AlphaVantageDataProviderModuleTests` |
...
| `backend\research_os\fmp_data_provider.py` | Financial Modeling Prep API client와 시장/재무 provider | `FmpDataProviderModuleTests` |
...
| `backend\research_os\telegram_brief_sender.py` | OpenClaw Portfolio Health/Top Movers/Watch Items 및 오늘 추천 텔레그램 dry-run 메시지 렌더링 | `TelegramBriefSenderTests`, `check_telegram_brief_sender.py` |
| `backend\research_os\telegram_brief_delivery.py` | 텔레그램 중요 브리프 발송 계획, message_id ledger, 저우선순위 삭제 후보 안전 분류 | `TelegramBriefSenderTests`, `check_telegram_brief_delivery.py` |
...
| `backend\research_os\recent_activity.py` | 최근 1주 자료용 출처별 compact helper, DART/추천 근거 링크, 저장 데이터 탐색 힌트 | `check_recent_weekly_brief.py`, `check_backend_module_health.py` |
...
| `backend\research_os\web_capture.py` | 웹 URL 안전성, URL 본문 추출, URL-only 예외 처리, 표시용 컨텍스트 | `WebCaptureRenderingTests` |
| `backend\research_os\web_fetch.py` | 웹 캡처용 HTTP header/retry, SEC urllib fallback helper | `WebCaptureRenderingTests` |
| `backend\research_os\web_capture_fallbacks.py` | 직접 수집 실패 시 공식 URL별 보조 요약 payload helper | `WebCaptureRenderingTests` |
...
| `backend\research_os\web_article_cleaning.py` | 웹 기사 제목/본문 노이즈 제거와 중복 라인 정리 helper | `WebCaptureRenderingTests` |
