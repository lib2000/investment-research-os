...
`bridge_status.json`은 OpenClaw가 가장 먼저 읽는 런타임 상태 파일이다. 원본 커밋/브랜치/dirty 상태, 컨텍스트 생성 시각, 최신 추천일, 한국/미국 추천 수, 텔레그램 반영 수, 민감정보 제외 확인, 완료 리포트 경로, 시작 안내 갱신 여부, 운영 명령 묶음, 핵심 파일 SHA256을 포함한다.
