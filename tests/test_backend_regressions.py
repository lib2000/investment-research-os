...
def load_openclaw_question_read_order_tool():
    tool_path = PROJECT_ROOT / "tools" / "check_openclaw_question_read_order.py"
    spec = spec_from_file_location("check_openclaw_question_read_order", tool_path)
    module = module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    return module
...
def load_openclaw_answer_samples_tool():
    tool_path = PROJECT_ROOT / "tools" / "check_openclaw_answer_samples.py"
    spec = spec_from_file_location("check_openclaw_answer_samples", tool_path)
    module = module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    return module
...
def load_openclaw_actual_answer_audit_tool():
    tool_path = PROJECT_ROOT / "tools" / "check_openclaw_actual_answer_audit.py"
    spec = spec_from_file_location("check_openclaw_actual_answer_audit", tool_path)
    module = module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    return module
...
def load_openclaw_answer_capture_cycle_tool():
    tool_path = PROJECT_ROOT / "tools" / "check_openclaw_answer_capture_cycle.py"
    spec = spec_from_file_location("check_openclaw_answer_capture_cycle", tool_path)
    module = module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    return module
...
def load_openclaw_actual_answer_capture_status_tool():
    tool_path = PROJECT_ROOT / "tools" / "check_openclaw_actual_answer_capture_status.py"
    spec = spec_from_file_location("check_openclaw_actual_answer_capture_status", tool_path)
    module = module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    return module
...
def load_portfolio_report_alert_task_status_tool():
    tools_dir = PROJECT_ROOT / "tools"
    if str(tools_dir) not in sys.path:
        sys.path.insert(0, str(tools_dir))
    tool_path = tools_dir / "check_portfolio_report_alert_task_status.py"
    spec = spec_from_file_location("check_portfolio_report_alert_task_status", tool_path)
    module = module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    return module
...
class ConsoleSmokeToolTests(unittest.TestCase):
    ...
    def test_console_public_ir_sec_status_lists_body_followups(self):
        ...
        self.assertIn("sec_exhibit_followup_count", console_source)
    ...
    def test_status_research_console_summarizes_automation_digest(self):
        ...
        self.assertIn("recommendationRankLabel", script_source)
        self.assertIn("최신 추천(기준일", script_source)
        self.assertIn("$recommendationRankLabel $($marketLabel) 1~3위", script_source)
        self.assertIn("오늘 추천 실행 필요", script_source)
        self.assertIn("today_records", script_source)
        ...
        self.assertIn("naver_market_close_journal_state.json", script_source)
        self.assertIn("국내 시장일지 자동 시도", script_source)
        self.assertIn("국내 시장일지 원천 상태", script_source)
        ...
        self.assertIn("stored_preview_mismatch_count", script_source)
        self.assertIn("stored_preview_mismatch_counts_by_market", script_source)
        ...
        self.assertIn("추천 저장/재계산 차이 요약", script_source)
        ...
        self.assertIn("source_freshness", script_source)
        self.assertIn("국민연금 원천 최신성", script_source)
        self.assertIn("freshness_status", script_source)
        self.assertIn("large_holding_staleness_days", script_source)
        self.assertIn("trusted_use", script_source)
        self.assertIn("국민연금 원천 지연 사유", script_source)
        self.assertIn("domestic_stock_latest_year", script_source)
        self.assertIn("large_holding_latest_base_date", script_source)
        ...
        self.assertIn("SEC Exhibit 후속", script_source)
    ...
    def test_openclaw_sync_script_validates_after_copy(self):
        ...
        self.assertIn("[switch]$RequireCompletionAudit", script_source)
        self.assertIn("[Console]::OutputEncoding", script_source)
...
class OfflineReadinessToolTests(unittest.TestCase):
    ...
    def test_offline_readiness_checks_portfolio_report_alert(self):
        ...
        self.assertIn("Telegram 보유 종목 리포트 예약작업", checks)
        self.assertEqual(
    ...
    def test_offline_readiness_checks_openclaw_investment_bridge(self):
        ...
        self.assertIn("OpenClaw 질문별 read-order smoke", checks)
        self.assertEqual(
            checks["OpenClaw 질문별 read-order smoke"],
            ["tools/check_openclaw_question_read_order.py", "--json"],
        )
        ...
        self.assertIn("OpenClaw 질문별 답변 샘플 smoke", checks)
        self.assertEqual(
            checks["OpenClaw 질문별 답변 샘플 smoke"],
            ["tools/check_openclaw_answer_samples.py", "--json"],
        )
        self.assertIn("OpenClaw 실제 답변 사후감사", checks)
        self.assertEqual(
            checks["OpenClaw 실제 답변 사후감사"],
            ["tools/check_openclaw_actual_answer_audit.py", "--json"],
        )
        self.assertIn("OpenClaw 답변 캡처 cycle", checks)
        self.assertEqual(
        ...
        self.assertEqual(
            checks["OpenClaw 실제 답변 캡처 상태"],
            ["tools/check_openclaw_actual_answer_capture_status.py", "--json"],
        )
...
class LlmBridgeStoreCheckToolTests(unittest.TestCase):
    def test_llm_bridge_store_check_supports_json_result_contract(self):
        source = (PROJECT_ROOT / "tools" / "check_llm_bridge_store.py").read_text(encoding="utf-8")

        self.assertIn('parser.add_argument("--json"', source)
        self.assertIn('"saved_count"', source)
        self.assertIn('"active_rag_connected_count"', source)
...
class OperationalReadinessToolTests(unittest.TestCase):
    ...
    def test_openclaw_status_summary_is_part_of_operational_readiness(self):
        tool = load_operational_readiness_tool()
        fake_module = SimpleNamespace(
            build_status_summary=lambda: {
                "status": "ok",
                "errors": [],
        ...
        with patch.dict(sys.modules, {"show_openclaw_bridge_status": fake_module}):
            result = tool.openclaw_status_summary_signal(PROJECT_ROOT)

        self.assertEqual(result["id"], "openclaw_status_summary")
        self.assertEqual(result["status"], "ok")
        self.assertEqual(result["score"], 100.0)
...
class DailyRecommendationRenderLayoutCheckToolTests(unittest.TestCase):
    def test_strict_errors_require_two_markets_six_cards_and_no_clipping(self):
        ...
        result = {
            ...
            "horizontalDetailLayout": False,
            "horizontalEvidenceLayout": False,
            "readableDetailWidth": False,
        ...
        self.assertIn("추천 상세 정보가 가로 그리드로 표시되지 않습니다.", errors)
        self.assertIn("추천 상세 정보 패널 폭이 가로 읽기에 부족합니다.", errors)
        self.assertIn("추천 근거 묶음이 가로 카드형으로 표시되지 않습니다.", errors)
...
class InterestSummaryRenderLayoutCheckToolTests(unittest.TestCase):
    def test_strict_errors_require_name_only_rows_and_clickable_details(self):
        ...
        result = {
            ...
            "tickerRecommendationDetailReady": False,
            "tickerRecommendationHorizontal": False,
        ...
        self.assertIn("관심종목 상세 추천 정보가 부족합니다.", errors)
        self.assertIn("관심종목 상세 추천 정보가 가로 패널로 표시되지 않습니다.", errors)
...
class AutomationStatusModuleTests(unittest.TestCase):
    ...
    def test_automation_digest_explains_duplicate_review_without_refresh_candidates(self):
        from research_os import automation_status
        ...
        stores = {
            ...
            "automation": {"dossier_count": 0, "failed_count": 0, "daily_brief_date": "2026-06-18"},
            "duplicate": {"duplicate_entry_count": 19, "duplicate_group_count": 1},
            "refresh_queue": {"status": "success", "candidate_count": 0},
            "daily_state": {},
        }
        runtime = SimpleNamespace(
            resolve_vault_dir=lambda _path: Path("vault"),
        ...
        self.assertTrue(any("재합성 후보는 없습니다" in item for item in digest["next_actions"]))
...
class NpsDomesticEquityAllocationMonitorTests(unittest.TestCase):
    ...
    def test_nps_portfolio_change_snapshot_uses_cached_large_holding_rows(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(snapshot["source_freshness"]["domestic_stock_latest_year"], "2022")
            self.assertEqual(snapshot["source_freshness"]["large_holding_latest_base_date"], "2022-07-01")
            self.assertTrue(snapshot["source_freshness"]["large_holding_stale_for_as_of"])
            self.assertEqual(snapshot["source_freshness"]["freshness_status"], "cache_stale_refresh_not_attempted")
            self.assertEqual(snapshot["source_freshness"]["large_holding_staleness_days"], 1461)
            self.assertEqual(snapshot["source_freshness"]["trusted_use"], "public_disclosure_pressure_only")
            self.assertEqual(snapshot["source_freshness"]["source_url_count"], 2)
    ...
    def test_openclaw_context_export_and_validation_exclude_sensitive_raw_state(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            (output_dir / "README.md").write_text(
                ...
                "  - KR#2 `002` 한국2 | score 98 | baseline 1002 KRW\n"
                "  - KR#3 `003` 한국3 | score 97 | baseline 1003 KRW\n"
                "  - US#1 `AAA` 미국1 | score 99 | baseline 1001 USD\n"
                "  - US#2 `BBB` 미국2 | score 98 | baseline 1002 USD\n"
                "  - US#3 `CCC` 미국3 | score 97 | baseline 1003 USD\n"
                "\n"
            ...
            first_read_text = (output_dir / "openclaw_first_read.md").read_text(encoding="utf-8")
            first_read_payload = json.loads((output_dir / "openclaw_first_read.json").read_text(encoding="utf-8"))
        ...
        self.assertIn('"today_answer_readiness_command": "python tools\\\\check_openclaw_today_answer_readiness.py --json"', exported_text)
        ...
        self.assertIn('"question_read_order_command": "python tools\\\\check_openclaw_question_read_order.py --json"', exported_text)
        ...
        self.assertIn('"today_answer_readiness_command": "python tools\\\\check_openclaw_today_answer_readiness.py --json"', manifest_text)
        ...
        self.assertIn('"question_read_order_command": "python tools\\\\check_openclaw_question_read_order.py --json"', manifest_text)
...
class OpenClawBridgeCompletionTests(unittest.TestCase):
    ...
    def test_completion_audit_writes_json_and_markdown_reports(self):
        ...
        self.assertIn("question_read_order", json_payload["operational_commands"])
        ...
        self.assertIn("## Status Summary Health", markdown)
        self.assertIn("- expected hash_status: `ok`", markdown)
        self.assertIn("- expected hash_checked_count: `14`", markdown)
        self.assertIn("- expected hash_mismatches: `[]`", markdown)
        ...
        self.assertIn("- question_read_order: `python tools\\check_openclaw_question_read_order.py --json`", markdown)
...
class OpenClawQuestionReadOrderTests(unittest.TestCase):
    def sample_read_order(self):
        return [
            "bridge_status.json",
            "openclaw_first_read.md",
            "openclaw_first_read.json",
    ...
    def write_bundle(self, openclaw_dir: Path, *, missing_us: bool = False):
        read_order = self.sample_read_order()
        ...
        for filename in read_order:
            ...
            if filename.endswith(".json"):
                ...
                path.write_text("# ok\n", encoding="utf-8")
        rows = [
            {"market": "KR", "rank": 1, "ticker": "001", "company_name": "한국1"},
            {"market": "KR", "rank": 2, "ticker": "002", "company_name": "한국2"},
            {"market": "KR", "rank": 3, "ticker": "003", "company_name": "한국3"},
            {"market": "US", "rank": 1, "ticker": "AAA", "company_name": "미국1"},
            {"market": "US", "rank": 2, "ticker": "BBB", "company_name": "미국2"},
            {"market": "US", "rank": 3, "ticker": "CCC", "company_name": "미국3"},
        ]
        if missing_us:
            rows = rows[:3]
        first_read = {
            "today_work_report": {"has_implementation_today": True, "commit_count": 10},
            "next_schedule": [{"time": "08:00", "task": "recommendations"}],
            "answer_correction": {"correct_basis": "bridge_status"},
            "latest_recommendations": rows,
        ...
        (openclaw_dir / "bridge_status.json").write_text(
            json.dumps({"read_order": read_order, "operational_commands": commands}, ensure_ascii=False),
            encoding="utf-8",
        )
        manifest = {
            "read_order": read_order,
            "status_summary_command": commands["status_summary"],
            "quick_health_command": commands["quick_health"],
            "today_answer_readiness_command": commands["today_answer_readiness"],
            "today_answer_quality_command": commands["today_answer_quality"],
            "priority_answer_quality_command": commands["priority_answer_quality"],
            "question_read_order_command": commands["question_read_order"],
    ...
    def test_question_read_order_validates_route_contracts(self):
        tool = load_openclaw_question_read_order_tool()

        with TemporaryDirectory() as tmp:
            openclaw_dir = Path(tmp)
            self.write_bundle(openclaw_dir)
    ...
    def test_question_read_order_rejects_incomplete_recommendation_route(self):
        tool = load_openclaw_question_read_order_tool()

        with TemporaryDirectory() as tmp:
            openclaw_dir = Path(tmp)
            self.write_bundle(openclaw_dir, missing_us=True)

            result = tool.build_result(openclaw_dir)

        self.assertEqual("failure", result["status"])
        self.assertTrue(any("latest_recommendations missing US top 3" in error for error in result["errors"]))


class OpenClawAnswerSamplesTests(unittest.TestCase):
    def test_answer_samples_build_question_specific_answers(self):
        tool = load_openclaw_answer_samples_tool()

        with TemporaryDirectory() as tmp:
            openclaw_dir = Path(tmp)
            OpenClawQuestionReadOrderTests().write_bundle(openclaw_dir)
            first_read = json.loads((openclaw_dir / "openclaw_first_read.json").read_text(encoding="utf-8"))
            first_read["today_work_report"]["summary"] = ["OpenClaw bridge reinforced"]
            first_read["latest_recommendation_date"] = "2026-07-05"
            first_read["latest_recommendations"] = [
                {"market": "KR", "rank": 1, "ticker": "001", "company_name": "한국1", "score": 99, "baseline_price": 1001, "currency": "KRW"},
                {"market": "KR", "rank": 2, "ticker": "002", "company_name": "한국2", "score": 98, "baseline_price": 1002, "currency": "KRW"},
                {"market": "KR", "rank": 3, "ticker": "003", "company_name": "한국3", "score": 97, "baseline_price": 1003, "currency": "KRW"},
                {"market": "US", "rank": 1, "ticker": "AAA", "company_name": "미국1", "score": 96, "baseline_price": 10.1, "currency": "USD"},
            ...
            (openclaw_dir / "investment_research_context.json").write_text(
                json.dumps(
                    {
                        "openclaw_knowledge_graph_blueprint": {
                            "schema": "openclaw_personal_knowledge_graph_blueprint_v1",
                            "seed_nodes": [
                                {"id": "concept.relu"},
                                {"id": "topic.graph_rendering_8000_nodes"},
...
class OpenClawActualAnswerAuditTests(unittest.TestCase):
    def write_bundle(self, openclaw_dir: Path):
        OpenClawQuestionReadOrderTests().write_bundle(openclaw_dir)
        first_read = json.loads((openclaw_dir / "openclaw_first_read.json").read_text(encoding="utf-8"))
        first_read["today_work_report"]["summary"] = ["OpenClaw bridge reinforced"]
        first_read["latest_recommendation_date"] = "2026-07-05"
    ...
    def test_actual_answer_audit_accepts_captured_answers(self):
        tool = load_openclaw_actual_answer_audit_tool()

        with TemporaryDirectory() as tmp:
            openclaw_dir = Path(tmp)
            self.write_bundle(openclaw_dir)
            answers_dir = openclaw_dir / "actual_answers"
            answers_dir.mkdir()
            (answers_dir / "today_work_report.md").write_text(
                "오늘 구현 작업 보고\n"
            ...
            (answers_dir / "recommendations_priority.json").write_text(
                json.dumps(
                    {
                        "route_id": "recommendations_priority",
                        "answer": (
                            "오늘 추천 종목\n"
                            "- 기준 파일: openclaw_first_read.json / bridge_status.json\n"
                            "- KR#1 001 한국1 score=99 baseline=1001 KRW\n"
                            "- KR#2 002 한국2 score=98 baseline=1002 KRW\n"
                            "- KR#3 003 한국3 score=97 baseline=1003 KRW\n"
                            "- US#1 AAA 미국1 score=96 baseline=10.1 USD\n"
                            "- US#2 BBB 미국2 score=95 baseline=10.2 USD\n"
                            "- US#3 CCC 미국3 score=94 baseline=10.3 USD\n"
                            "\n"
                            "중요 메시지\n"
                            "- 텔레그램 즐겨찾기 수집: 10건\n"
                        ),
            ...
            result = tool.build_result(openclaw_dir, answers_dir, require_answers=True)
            rendered = tool.render_text(result)

        self.assertEqual("ok", result["status"])
        self.assertEqual(2, result["audited_count"])
        self.assertIn("recommendations_priority: ok", rendered)

    def test_actual_answer_audit_rejects_stale_claims(self):
        tool = load_openclaw_actual_answer_audit_tool()
        ...
        with TemporaryDirectory() as tmp:
            ...
            answers_dir = openclaw_dir / "actual_answers"
            answers_dir.mkdir()
            (answers_dir / "today_work_report.md").write_text("오늘 구현 작업 없음\n", encoding="utf-8")
...
class OpenClawActualAnswerCaptureTests(unittest.TestCase):
    def test_capture_actual_answer_writes_json_and_runs_audit(self):
        tool = load_openclaw_actual_answer_capture_tool()
        ...
        self.assertEqual("ok", result["status"])
        self.assertEqual("today_work_report", saved["route_id"])
        self.assertEqual("openclaw_actual_answer_capture_v1", saved["schema"])
        self.assertEqual("ok", result["audit"]["status"])
        self.assertIn("audit: ok", rendered)

    def test_capture_actual_answer_rejects_unknown_route(self):
        tool = load_openclaw_actual_answer_capture_tool()

        with TemporaryDirectory() as tmp:
            with self.assertRaises(AssertionError):
                tool.build_result(route_id="unknown", answer="hello", openclaw_dir=Path(tmp))


class OpenClawActualAnswerCaptureStatusTests(unittest.TestCase):
    def test_capture_status_reports_latest_capture_and_route_counts(self):
        capture_tool = load_openclaw_actual_answer_capture_tool()
        status_tool = load_openclaw_actual_answer_capture_status_tool()

        with TemporaryDirectory() as tmp:
            openclaw_dir = Path(tmp)
            OpenClawActualAnswerAuditTests().write_bundle(openclaw_dir)
            capture_tool.build_result(
                route_id="today_work_report",
                answer=(
                    "오늘 구현 작업 보고\n"
                    "- 기준: openclaw_first_read.json / bridge_status.json / today_work_report\n"
            ...
            result = status_tool.build_result(
                openclaw_dir,
                require_recent=True,
                require_routes=["today_work_report"],
            )
...
class OpenClawAnswerCaptureCycleTests(unittest.TestCase):
    def test_capture_cycle_dry_run_reports_pending_without_moving(self):
        tool = load_openclaw_answer_capture_cycle_tool()
        ...
        with TemporaryDirectory() as tmp:
            ...
            result = tool.build_result(openclaw_dir, collect=False)
            rendered = tool.render_text(result)
            pending_exists = pending_file.exists()

        self.assertEqual("ok", result["status"])
        self.assertEqual(1, result["collector"]["pending_count"])
        self.assertEqual(0, result["collector"]["captured_count"])
        self.assertTrue(pending_exists)
        self.assertIn("pending_count: 1", rendered)

    def test_capture_cycle_collects_and_writes_state(self):
        tool = load_openclaw_answer_capture_cycle_tool()

        with TemporaryDirectory() as tmp:
            openclaw_dir = Path(tmp) / "openclaw"
            openclaw_dir.mkdir()
            OpenClawActualAnswerAuditTests().write_bundle(openclaw_dir)
            pending_dir = openclaw_dir / "pending_actual_answers"
            pending_dir.mkdir()
            (pending_dir / "today_work_report.md").write_text(
                "오늘 구현 작업 보고\n"
                "- 기준: openclaw_first_read.json / bridge_status.json / today_work_report\n"
                "- 오늘 반영 커밋: 10건\n"
                "- OpenClaw bridge reinforced\n"
            ...
            result = tool.build_result(openclaw_dir, collect=True, write_state=True, state_file=state_file)
        ...
        self.assertEqual(1, result["collector"]["captured_count"])
        self.assertEqual(1, result["capture_status"]["capture_count"])
        self.assertEqual(1, len(captures))
...
class OpenClawWslAnswerContextTests(unittest.TestCase):
    def test_wsl_answer_context_reports_pa_sessions_and_today_readiness(self):
        ...
        payload = {
            ...
            "actual_answer_audit": {"status": "ok", "audited_count": 0},
            "actual_answer_capture_status": {"status": "ok", "capture_count": 1},
            "sessions": {
                "items": {
        ...
        self.assertIn("actual answer audit: ok", rendered)
        self.assertIn("actual answer capture status: ok", rendered)
...
class OpenClawQuickHealthTests(unittest.TestCase):
    def test_quick_health_combines_openclaw_specific_checks(self):
        ...
        fake_question_read_order = SimpleNamespace(
            build_result=lambda openclaw_dir: {
                "status": "ok",
                "route_count": 4,
                "first_read_declared_route_count": 4,
                "errors": [],
            }
        )
        ...
        fake_answer_samples = SimpleNamespace(
            build_result=lambda openclaw_dir: {
                "status": "ok",
                "sample_count": 4,
                "generated_at": "2026-07-05T18:00:00+09:00",
        ...
        fake_actual_answer_audit = SimpleNamespace(
            build_result=lambda openclaw_dir: {
                "status": "ok",
                "audited_count": 0,
                "answers_dir": "C:/openclaw/data/investment_research/actual_answers",
                "errors": [],
            }
        )
        fake_answer_capture_cycle = SimpleNamespace(
            build_result=lambda openclaw_dir, collect=False: {
                "status": "ok",
                "errors": [],
                "collect": collect,
                "collector": {"pending_count": 0, "captured_count": 0, "failed_count": 0},
        ...
        fake_modules = {
            ...
            "check_openclaw_question_read_order": fake_question_read_order,
        ...
        self.assertIn("question_read_order: ok", rendered)
    ...
    def test_quick_health_fails_on_status_hash_mismatch(self):
        ...
        fake_question_read_order = SimpleNamespace(build_result=lambda *args, **kwargs: {"status": "ok", "errors": []})
