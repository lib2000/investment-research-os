...
Write-Host "연결 복구: .\scripts\restart-research-backend.ps1 -Port 8001"
Write-Host "복구 후 열기: .\scripts\enter-investment-research-os.ps1 -RestartBackend -OpenConsole"
Write-Host "상태 점검:  .\tools\status_research_console.ps1 -Strict"
Write-Host "콘솔 주소:  http://127.0.0.1:8001/console/index.html"
Write-Host "매일 추천: 콘솔 대시보드의 '오늘 한국/미국 추천 1~3위' 또는 저장 데이터 탭에서 확인"

if ($RestartBackend) {
  & (Join-Path $ProjectRoot "scripts\restart-research-backend.ps1") -Port 8001
}
