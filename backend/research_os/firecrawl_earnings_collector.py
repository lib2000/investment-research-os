"""Firecrawl earnings payload helpers for Market Signal Graph."""
...
DESIGN_NAME = "firecrawl_earnings_collector_v1"
SOURCE_PLATFORM = "firecrawl_earnings"
SOURCE_KIND = "earnings"
CHANNEL = "web"
COLLECTOR = "firecrawl"
TARGET_TYPE = "company_earnings"


@dataclass(frozen=True)
class FirecrawlEarningsInput:
    company: str
    ticker: str
    raw_url: str
...
def _safe_url(raw_url: str, resolved_url: str | None = None) -> str:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ValueError("Firecrawl earnings payload requires a public http/https URL.")
    return candidate


def normalize_earnings_text(value: Any, *, max_chars: int = 1600) -> str:
...
def _payload_containers(item: dict[str, Any]) -> list[dict[str, Any]]:
    containers = [item]
    for key in ("data", "scrape", "firecrawl", "result", "earnings"):
...
def _coerce_earnings_input(item: dict[str, Any]) -> FirecrawlEarningsInput:
...
def build_firecrawl_earnings_signal_payload(item: FirecrawlEarningsInput | dict[str, Any]) -> dict[str, Any]:
    if isinstance(item, dict):
        item = _coerce_earnings_input(item)
    ...
    title = _safe_text(item.title) or f"{company} earnings update"
    normalized_text = normalize_earnings_text(item.text if item.text is not None else item.markdown)
    if not normalized_text:
        normalized_text = f"{company} earnings page captured by Firecrawl for {fiscal_period or event_date or 'latest period'}."
    ...
    return {
        "source_name": f"{company}_earnings",
        ...
        "author": f"{company} Investor Relations",
...
def normalize_firecrawl_earnings_inputs(data: Any) -> list[FirecrawlEarningsInput | dict[str, Any]]:
    if isinstance(data, list):
        return [item for item in data if isinstance(item, dict)]
    if isinstance(data, dict):
        for key in ("items", "sources", "results", "payloads", "earnings"):
            value = data.get(key)
...
def build_firecrawl_earnings_batch_result(items: list[FirecrawlEarningsInput | dict[str, Any]]) -> dict[str, Any]:
    results: list[dict[str, Any]] = []
    for index, item in enumerate(items, start=1):
        try:
            payload = build_firecrawl_earnings_signal_payload(item)
            errors: list[str] = []
