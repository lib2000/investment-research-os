...
def build_next_schedule() -> list[dict]:
    return [
        ...
        {
            "time": "07:10",
            "task": "가족 오늘의 한 종목 리서치 카드 생성",
            "status": "enabled; reuses the saved 07:00 recommendation without another market lookup",
            "scheduler": "backend_daily_gate",
            "command": "python tools\\check_operational_schedule_status.py --json --allow-warnings",
        },
        {
...
def render_markdown(context: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "- 07:00 추천, 07:10 가족 한 종목 카드, 07:20/22:00 백엔드 운영 스케줄 상태는 `python tools\\check_operational_schedule_status.py --json --allow-warnings`로 확인합니다.",
