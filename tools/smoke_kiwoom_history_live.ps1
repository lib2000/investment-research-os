...
function Convert-ToKiwoomDate {
  ...
  throw "TradeDate는 YYYY-MM-DD 또는 YYYYMMDD 형식이어야 합니다: $Value"
}
...
if (-not $ConfirmLiveApi) {
  throw @"
키움 라이브 과거 거래 스모크는 실제 키움 API와 로컬 DB를 사용합니다.
실행하려면 명시적으로 -ConfirmLiveApi 를 붙이세요.

예:
  .\tools\smoke_kiwoom_history_live.ps1 -TradeDate $TradeDate -ConfirmLiveApi

출력에는 토큰 원문과 계좌번호를 표시하지 않습니다.
"@
}

$kiwoomDate = Convert-ToKiwoomDate -Value $TradeDate
$displayDate = Convert-ToDisplayDate -Yyyymmdd $kiwoomDate
...
$encodedDate = [uri]::EscapeDataString($displayDate)

Write-Host "키움 라이브 1일 과거 거래 스모크를 시작합니다."
Write-Host "API: $baseUrl"
Write-Host "조회일: $displayDate"
Write-Host "주의: 이 스크립트는 1일 조회만 실행합니다."
...
$startResponse = Invoke-ApiJson `
  ...
  -Method Post `
  -Uri "$baseUrl/api/v1/sync/kiwoom/history/start?start_date=$encodedDate&end_date=$encodedDate" `
  -Headers $authHeaders
...
Write-Host ""
Write-Host "키움 라이브 1일 과거 거래 스모크 결과"
