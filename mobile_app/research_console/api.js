...
export async function runPublicIrSecFirecrawlMonitorDryRun(accessToken) {
  try {
    return request("/api/v1/public-ir-sec/firecrawl-monitor/dry-run", {
      method: "POST",
      accessToken,
      timeoutMs: 90000,
    });
  } catch (error) {
    console.error("Firecrawl Monitor dry-run 실행 중 오류 발생:", error);
    return null;
  }
}
...
export async function fetchNpsDomesticEquityRebalancePlan(accessToken, portfolioName = "__all__") {
  return request(
    `/api/v1/portfolios/${encodeURIComponent(portfolioName || "__all__")}/nps-domestic-equity-rebalance-plan`,
    {
      method: "GET",
      accessToken,
    }
...
export async function fetchTossPaperEvaluation(accessToken, days = 7) {
  const params = new URLSearchParams({ days: String(days) });
  return request(`/api/v1/brokerage/toss/workflow/paper-evaluation?${params.toString()}`, {
    method: "GET",
    accessToken,
  });
}
